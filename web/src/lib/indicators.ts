...
export const DISCOVER_INDICATORS: Indicator[] = [
  ...
  {
    label: "效应",
    short: "领先股每涨 1%，另一只在 k 天后平均涨多少 % —— 以及这解释了它多少涨跌",
    what: "在后半程（没参与挑选的那一半）上，把滞后 k 天的领先股收益率回归到另一只上："
      + "斜率是每 1% 对应多少 %，后面的百分比是 R²。",
    reads: "p 值只说「有关系」，不说「多大」。这一栏说多大。在 A 股日线上，"
      + "实测斜率多在 0.05–0.2 之间，R² 不到 2% —— 也就是领先股涨 5%，"
      + "另一只在几天后平均涨 0.3%–1%，而且这点关系只解释了它日涨跌的百分之一二。",
    caveat: "两点都要留神。一是这只是平均值：在领先股单日涨跌超过 4% 的那些日子里，"
      + "另一只几天后同方向的比例实测在 41%–66% 之间 —— 接近抛硬币。"
      + "把它当成「A 涨停所以 B 明天会涨」来用，正是这一栏要阻止的事。"
      + "二是它算在后半程上，而配对能通过也正是因为后半程 —— 所以这个数字"
      + "偏大，是上限而不是期望值。",
  },
  {
    label: "一致",
